...
exports.detectProjectAndProduct = detectProjectAndProduct;
exports.buildMeaningfulFindings = buildMeaningfulFindings;
exports.prioritizeFindings = prioritizeFindings;
exports.buildNextActions = buildNextActions;
exports.buildAttachmentInsights = buildAttachmentInsights;
function inferProjectCategory(text, iterationName) {
...
function inferArtifactType(fileName, mimeType) {
...
function inferProductName(text, fallback) {
...
function inferProjectName(iterationName, fallback) {
...
function detectProjectAndProduct(params) {
...
function buildMeaningfulFindings(params) {
...
function prioritizeFindings(findings) {
    ...
    return deduped.slice(0, 8).map((content) => {
        ...
        if (/风险|阻断|失败|回滚|故障|blocked|failed/.test(lower)) {
            return { priority: "P0", content, reason: "涉及发布风险或阻断项" };
        }
        if (/变更|新增|差异|接口|数据|验收|覆盖/.test(lower)) {
            return { priority: "P1", content, reason: "涉及范围/实现/验收关键变更" };
        }
        return { priority: "P2", content, reason: "辅助信息，建议跟进确认" };
    });
}
...
function buildAttachmentInsights(params) {
    ...
    const safeCharacteristics = uniqueStrings(characteristics).length > 0 ? uniqueStrings(characteristics) : ["以需求范围调整为主，建议结合迭代目标进一步确认细节"];
    const versionChangeSummary = `相较上版新增 ${added.length} 项、变更 ${changed.length} 项、移除 ${removed.length} 项；重点变化：${[...added.slice(0, 2), ...changed.slice(0, 2), ...removed.slice(0, 1)].join("；") || "暂无结构化差异"}。`;
    const limitations = [];
