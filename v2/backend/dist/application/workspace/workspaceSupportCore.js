...
exports.createDefaultProjectRepository = createDefaultProjectRepository;
...
function defaultRepositoryLayout() {
    return [
...
function createDefaultProjectRepository(project) {
...
function normalizeCodeLink(iteration, link) {
    ...
    return {
        ...
        tag: link.tag || (iteration.version ? `v${(0, versioning_1.normalizeThreePartVersion)(iteration.version)}` : `iter-v${iteration.id}`),
        commit: link.commit || "",
        pr: link.pr || "",
...
function normalizeChangeBoundary(boundary) {
    return {
...
function normalizeChangeControl(control) {
    return {
        ...
        lastReleaseReviewDecision: control?.lastReleaseReviewDecision === "go" || control?.lastReleaseReviewDecision === "caution" || control?.lastReleaseReviewDecision === "block"
...
function normalizeIteration(iteration) {
    ...
    return {
        ...iteration,
        version: (0, versioning_1.normalizeThreePartVersion)(iteration.version),
        goals,
...
function buildMergedIterationPayload(payload, project, previous) {
    ...
    const assessment = {
        baselineIterationId: previous?.id ?? null,
        baselineIterationName: previous?.name ?? "无基线",
        currentSummary: payload.aiSummary?.trim() || `基于项目「${project?.name ?? "未命名项目"}」元信息，${payload.name} 继承上版本上下文并进入执行。`,
        deltaInScope: [
