...
function resolvePromptPath(roleKey) {
    const v2Path = (0, node_path_1.resolve)(process.cwd(), "prompts", `agent.${roleKey}.v2.md`);
    if ((0, node_fs_1.existsSync)(v2Path)) {
        return v2Path;
    }
    return (0, node_path_1.resolve)(process.cwd(), "prompts", `agent.${roleKey}.v1.md`);
}
...
function buildIterationAgentPlan(params) {
    ...
    const boundaryDigest = boundary && (boundary.requirementRefs.length > 0 || boundary.componentRefs.length > 0 || boundary.codePaths.length > 0)
        ...
        return {
            ...
            prompts: [
                buildPrompt({
                    ...
                    expectedOutput: "JSON: {summary, stagePlan:[{stage,goal,entryCriteria,exitCriteria,inBoundary:boolean}], blockers:[{id,reason,severity,evidence}], unknowns[], humanConfirmation:{required,questions[]}, nextAction}"
                })
            ]
        };
    }
    return {
        ...
        prompts: [
            buildPrompt({
                ...
                context: contextWithControl,
                expectedOutput: "JSON: {diff:{added[],changed[],removed[]}, assumptions[], risks:[{item,level,evidence}], domainTerms:[{term,definition,evidence}], mappingHints:[{requirement,component,codePath,evidence}], unknowns[], clarificationQuestions[]}"
            }),
            buildPrompt({
                ...
                context: contextWithControl,
                expectedOutput: "JSON: {workPackages:[{id,title,owner,priority,dependsOn[],acceptanceCriteria[],inBoundary:boolean,evidence}], criticalPath[], outOfBoundaryWork[]}"
            }),
            buildPrompt({
                ...
                goal: "输出开发执行计划与回滚策略",
                context: contextWithControl,
                expectedOutput: "JSON: {implementationSteps:[{step,targets[],boundaryCheck,evidence}], codeChangePlan:[{path,changeType,reason,inBoundary:boolean}], rollbackPlan:[{trigger,action}], releaseGates[], stopConditions[]}"
            }),
            buildPrompt({
                agentId: "agent-qa-1",
                role: "qa-reviewer",
                ...
                context: contextWithControl,
                expectedOutput: "JSON: {testMatrix:[{type,caseId,focus,expected,evidence}], regressionsToWatch[], releaseDecision:{pass:boolean,reason,blockers[]}, recommendedTransition, unknowns[]}"
            }),
            buildPrompt({
                ...
                context: contextWithControl,
                expectedOutput: "JSON: {boundary:{requirementRefs[],componentRefs[],codePaths[],note}, violations:[{item,reason,evidence}], confirmationChecklist[], unknowns[]}"
            }),
            buildPrompt({
                ...
                goal: "输出发布前运维归因与回滚建议",
                context: contextWithControl,
                expectedOutput: "JSON: {hypotheses:[{priority,item,evidence}], triageSteps:[{step,expectedSignal,fallback}], rollbackDecision:{shouldRollback,reason,trigger}, postmortemChecklist[], unknowns[]}"
            })
        ]
    };
}
