...
function buildCoreAnalysisSystemPrompt(isCompact: boolean): string {
  return [
    ...
    "6) deepInsights: {coverage:{consideredFiles,analyzedFiles,partialFiles,failedFiles,coveragePercent}, fileInsights:[{path,fileName,mimeType,size,kind,status,mainContent,requiredWork,iterationValue,summary,keyPoints[],risks[],optimizeItems[],keepItems[],recommendedActions[],openQuestions[],citations[],confidence}], crossFileInsights:{themes(主题)[],conflicts(冲突)[],gaps(差距)[],recommendations(建议)[],conflictChains(冲突链)[],rootCauses(根因)[],impactScope(影响范围)[],decisionSuggestions(决策建议)[]}}",
    "7) traceabilityMap: {requirementToComponent:[{requirement,components[],evidence}], componentToCode:[{component,codePaths[],evidence}], requirementToCode:[{requirement,codePaths[],evidence}], coverageScore:0-100, mappingConfidence:'high'|'medium'|'low', unmappedRequirements(未映射需求)[], conflicts(映射冲突)[], gaps(覆盖缺口)[]}",
    "8) executableConstraints: {componentWhitelist[], codePathWhitelist[], acceptanceChecks[], gateRules[]}",
    "9) domainKnowledge: {terms:[{term,definition,mappedTo:{pages[],apis[],entities[],codePaths[]},evidence,bindingStrength:'high'|'medium'|'low'}], rules(业务规则)[], unknowns(待确认项)[]}",
    "10) versionDiffDetailed: {summary, impactScope(影响范围)[], riskPoints(风险点)[], added:[{dimension,item,impact,risk}], changed:[{dimension,item,impact,risk}], removed:[{dimension,item,impact,risk}]}",
