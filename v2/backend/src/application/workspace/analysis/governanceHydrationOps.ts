...
import type { parseGovernanceInsightsCandidate, parseReportQualityCandidate } from './governanceOps';
import type { parseReleaseReviewCandidate } from './releaseReviewOps';
...
export function hydrateReportQualityCandidate(
  ...
  const missingItems = uniqueTrimmed([
    ...
    ...(params.businessConfirmation.coreIntent ? [] : ["缺少核心业务目标"]),
    ...(params.businessConfirmation.evidenceRefs.length > 0 ? [] : ["缺少支撑材料或证据来源"]),
    ...(params.deepInsights.crossFileInsights.rootCauses.length > 0 ? [] : ["缺少跨模块影响分析"]),
    ...(params.deepInsights.crossFileInsights.decisionSuggestions.length > 0 ? [] : ["缺少可执行的决策建议"]),
    ...(params.clarificationQuestions.length > 0 ? ["存在未澄清的业务问题"] : [])
  ]);
  const actionRequired = uniqueTrimmed([
    ...candidate.actionRequired,
    ...(params.businessConfirmation.evidenceRefs.length > 0 ? [] : ["补充相关业务材料或依据"]),
    ...(params.deepInsights.crossFileInsights.rootCauses.length > 0 ? [] : ["补充跨模块影响的根因说明"]),
    ...(params.deepInsights.crossFileInsights.decisionSuggestions.length > 0 ? [] : ["补充可落地的决策建议"]),
    ...(params.clarificationQuestions.length > 0 ? params.clarificationQuestions.map((item) => `待澄清：${item}`) : [])
  ]);
