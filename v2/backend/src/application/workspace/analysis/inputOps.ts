...
function buildBatchContexts(
  limitedFiles: Array<{ path: string; fileName: string; mimeType: string; excerpt: string }>,
  batchSize: number
): string[] {
  const maxBatches = Math.max(4, Math.ceil(limitedFiles.length / batchSize));
  return Array.from({ length: Math.ceil(limitedFiles.length / batchSize) }, (_, index) => {
    ...
    const excerptPart = batch
      ...
      .join("\n\n");
    return [`第 ${index + 1} 批`, `manifest:\n${manifestPart}`, excerptPart ? `excerpt:\n${excerptPart}` : ""].filter(Boolean).join("\n\n");
  }).slice(0, maxBatches);
}

function composeFolderExcerptPayload(
  rawFiles: Array<{ path: string; fileName: string; mimeType: string; size: number; excerpt: string }>,
  input: AttachmentUploadInput,
  guardrails: AttachmentExcerptGuardrails,
  folderSelection: FolderSelectionDecision | null | undefined,
  inlineVisionPayloads: Array<{ path: string; mimeType: string; dataUrl: string }>
): AttachmentExcerptPayload {
  const consideredFiles = rawFiles.length;
  const ignoredFiles = Array.isArray(folderSelection?.ignoredFiles) ? folderSelection.ignoredFiles.slice(0, 40) : [];
  ...
  const manifest = limitedFiles
    ...
    .map((item, index) => `[${index + 1}] ${item.path || item.fileName} (${item.mimeType})`)
    .join("\n");
  ...
  const batchContexts = buildBatchContexts(limitedFiles, 30);
  const text = [
    `文件夹：${folderLabel}`,
    `manifest:\n${manifest}`,
    excerpts ? `excerpt:\n${excerpts}` : "",
  ...
  return {
    ...
    digest: (input.excerptDigest || "").trim() ||
      `策略：文件夹分批，考虑 ${consideredFiles} 个，纳入 ${limitedFiles.length} 个，文本 ${textFiles} 个，二进制 ${binaryFiles} 个，${sampled ? "已采样" : "未采样"}`,
    strategy: "folder-batch",
    fileStats: { totalFiles: limitedFiles.length, textFiles, binaryFiles },
    fileSelection: {
      consideredFiles, includedFiles: limitedFiles.length, skippedNoiseFiles, skippedEmptyFiles,
      sampled, sampleReason,
...
function buildSingleFilePayload(
  text: string, digest: string, strategy: string, fileName: string
): AttachmentExcerptPayload {
  return {
    text, digest, strategy,
    fileStats: { totalFiles: 1, textFiles: text.length > 0 ? 1 : 0, binaryFiles: text.length > 0 ? 0 : 1 },
    fileSelection: {
      consideredFiles: 1, includedFiles: 1, skippedNoiseFiles: 0, skippedEmptyFiles: 0,
      sampled: false, sampleReason: "", includedPaths: [fileName], ignoredFiles: []
    },
    batchContexts: []
  };
}
...
export function composeAttachmentExcerpt(
  ...
  if (rawFiles.length > 0 || input.sourceType === "folder") {
    return composeFolderExcerptPayload(rawFiles, input, guardrails, folderSelection, inlineVisionPayloads);
  }
  ...
  if (chunks.length === 0) {
    const baseText = baseExcerpt.slice(0, 20000) ||
      (inlineVisionPayloads.length > 0
        ? inlineVisionPayloads.map((item, index) => `[image ${index + 1}] ${item.path || fileName} (${item.mimeType || "image"})`).join("\n")
        : "");
    return buildSingleFilePayload(baseText, digest || `策略：${strategy}，分片 0`, strategy, fileName);
  }
  ...
  return buildSingleFilePayload(combined.slice(0, 50000), digest || `strategy=${strategy};chunks=${chunks.length}`, strategy, fileName);
}
