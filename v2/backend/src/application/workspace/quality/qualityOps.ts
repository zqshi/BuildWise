...
export function buildIterationReleaseReviewOp(repo: WorkspaceRepository, iterationId: number): IterationReleaseReviewResponse | null {
  ...
  if (summary.failed > 0 || summary.blocked > 0) blockers.push(`测试矩阵存在失败/阻断用例（失败 ${summary.failed} 个、阻断 ${summary.blocked} 个）。`);
  ...
  if (summary.total > 0 && summary.coverage < 100) warnings.push(`仍有未执行测试用例（覆盖率 ${summary.coverage}%）。`);
  if (summary.executed > 0 && summary.passRate < 90) warnings.push(`测试通过率偏低（通过率 ${summary.passRate}%）。`);
  if (traceabilityCoverage >= 40 && traceabilityCoverage < 70) warnings.push(`需求映射覆盖率待提升（${traceabilityCoverage}%）。`);
  if (changeControl?.lastReleaseReviewDecision === "caution") warnings.push("上次发布评审为「谨慎发布」，建议复核。");
  ...
  if (summary.total > 0 && summary.coverage < 100) recommendations.push("将待执行用例全部完成，再申请发布。");
  if (traceabilityCoverage < 70) recommendations.push("补齐需求-组件-代码映射，确保关键需求有代码落点。");
  if (summary.passRate < 90) recommendations.push("优先修复失败和阻断用例，再进行回归验证。");
  ...
  // 自动回写 changeControl，确保 synthesizeReleaseReview 能读到数据
  const generatedAt = new Date().toISOString();
  const latest = repo.findIteration(iterationId);
  if (latest) {
    const latestNormalized = normalizeIteration(latest);
    const cc = latestNormalized.changeControl ?? defaultIterationChangeControl();
    latestNormalized.changeControl = {
      ...cc,
      lastReleaseReviewDecision: decision,
      lastReleaseReviewReason: blockers[0] || warnings[0] || "",
      lastReleaseReviewBlockers: Array.from(new Set(blockers)).slice(0, 20),
      lastReleaseReviewScore: score,
      lastReleaseReviewUpdatedAt: generatedAt,
      lastOpsRollbackSuggested: rollback.shouldRollback
    };
