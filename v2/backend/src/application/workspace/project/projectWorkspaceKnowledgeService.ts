...
function buildKnowledgeDocPaths(memoryDir: string, dailyDir: string, knowledgeRoot: string, now: string) {
  return {
...
function writeShardIndex(shardDir: string, indexDir: string, project: Project, kb: NonNullable<Project["knowledgeBase"]>, iterations: Iteration[], now: string, projectId: number) {
  ...
  const indexedShards: IndexedProjectKnowledgeShard[] = shards.map((item) => ({
    ...item, vector: buildVector(`${item.title}\n${item.content}\n${item.tags.join(" ")}`),
  }));
  writeTextFile(join(indexDir, "shards.json"), JSON.stringify({
    version: INDEX_VERSION, projectId, updatedAt: now, dimensions: VECTOR_DIMENSIONS, shards: indexedShards,
  }, null, 2));
  return indexedShards.length;
}
...
export function syncProjectWorkspaceKnowledge(repo: WorkspaceRepository, projectId: number): ProjectWorkspaceSyncResult | null {
  ...
  if (!project || !binding || !binding.workspacePath.trim()) return null;
  ...
  ensureDir(memoryDir); ensureDir(dailyDir); ensureDir(shardDir); ensureDir(indexDir);
  const documents = buildKnowledgeDocPaths(memoryDir, dailyDir, knowledgeRoot, now);
  writeKnowledgeMemoryFiles(documents, project, kb, iterations, now);
  const shardCount = writeShardIndex(shardDir, indexDir, project, kb, iterations, now, projectId);
  return { projectId, workspacePath, documentsWritten: Object.values(documents), shardCount, syncedAt: now };
}
