...
const coachPromptFallback: CoachPromptTemplate = {
  systemPrompt: [
    ...
    "当你要输出以下类型的详细内容时，必须声明对应的 artifact id，由系统生成交付物卡片，对话正文只做简短说明（2-4句话概括要点和下一步建议）：",
    "- API 接口设计、端点列表、请求/响应结构 → 声明 api-specification",
    "- 数据库表结构、ER 关系、字段定义 → 声明 database-design",
    "- 技术架构方案、模块拆分、技术选型 → 声明 technical-architecture",
    "- 产品需求文档、用户故事、功能清单 → 声明 product-requirements-doc",
    "- 测试用例、测试矩阵 → 声明 test-matrix",
    "- 设计规范、交互流程、原型说明 → 声明 design-spec",
    "- 发布计划、部署方案 → 声明 deployment-plan",
    "违反此规则的输出（在对话正文中包含超过 5 行的结构化技术内容）视为格式错误。",
    "",
    "完整示例：",
    "---",
    ...
    "---",
    "",
    "声明交付物的示例（注意对话正文只做简短说明，不展开详细内容）：",
    "---",
    "API 设计方案已经基本成型了。核心涉及文档管理、FAQ、文件上传和统计这四个模块，一共 15 个接口。我帮你生成 API 规格文档，你看一下有没有遗漏或需要调整的。",
    "",
    '<!-- coach:{"intent":"plan","execution":{"action":"none","instruction":"","apply":false,"artifacts":["api-specification"]},"guidance":{"uploadRecommended":false,"suggestedUploadTypes":[],"suggestedActions":["确认接口覆盖完整性","确认鉴权方案"],"clarificationChecklist":[]}} -->',
    "---",
    "",
...
export async function coachIterationConversationOp(
  ...
  const context = [
    ...
    recentMessages.length > 0
      ? `最近的对话：\n${recentMessages.map((item, idx) => `  ${idx + 1}. ${item.role === "user" ? "用户" : "教练"}：${sanitizeForCoachContext(item.content.slice(0, 400).replace(/\s+/g, " "))}`).join("\n")}`
      : "",
  ...
  try {
    ...
    try {
      ...
      console.warn("[coach-ops] First LLM attempt failed, retrying once",
