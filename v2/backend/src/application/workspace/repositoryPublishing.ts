...
async function requestGitHub(
  ...
  init: RequestInit,
  token: string
): Promise<{ status: number; body: GitHubPrResponse | null }> {
  ...
  let body: GitHubPrResponse | null = null;
  try {
    body = (await res.json()) as GitHubPrResponse;
  } catch {
    body = null;
  }
...
async function createOrLoadPr(input: PublishIterationInput): Promise<{ prUrl: string; prNumber: number }> {
  ...
  if (existing.status === 200) {
    const list = (existing.body as unknown as GitHubPrResponse[]) || [];
