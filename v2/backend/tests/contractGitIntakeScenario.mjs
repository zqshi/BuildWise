...
export async function runContractGitIntakeScenario(options = {}) {
  const defaultPort = process.env.STORAGE_BACKEND === "sqlite" ? 6912 : 6910;
  const testPort = Number(process.env.CONTRACT_GIT_INTAKE_TEST_PORT || defaultPort);
  const baseUrl = (options.baseUrl || process.env.CONTRACT_BASE_URL || `http://127.0.0.1:${testPort}`).replace(/\/+$/, "");
  const fixtureDir = options.fixtureDir || mkdtempSync(path.join(tmpdir(), "buildwise-contract-git-intake-"));
  const app = options.app || null;
  const defaultHeaders = options.defaultHeaders || {
    "x-role": "owner",
    "x-user-id": "contract-owner"
  };
  const strictCoachBranchReason = options.strictCoachBranchReason !== false;
  const client = createHttpTestClient({
    app,
    baseUrl,
    defaultHeaders,
    tokenByRole: options.tokenByRole,
    requestTimeoutMs: Number(process.env.CONTRACT_REQUEST_TIMEOUT_MS || 120000)
  });
  let server = null;
  let stdout = "";
  let stderr = "";
  ...
  const { assert, getJson, request, waitForHealth } = client;
  ...
  if (strictCoachBranchReason) {
    ...
    assert(typeof syncAlert.payload?.reply === "string" && syncAlert.payload.reply.length > 0, "sync flow should return a coach reply");
  }
  } finally {
    if (server) {
      server.kill("SIGTERM");
