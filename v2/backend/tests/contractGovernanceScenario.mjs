...
export async function runContractGovernanceScenario(context, state) {
  ...
  const statusPayload = await getJson("/api/v1/status");
  ...
  const roles = await getJson("/api/v1/governance/roles");
  ...
  const permissionPoints = await getJson("/api/v1/governance/permission-points");
  ...
  const smsRequestBeforeBinding = await request("/api/v1/auth/sms/request", {
    method: "POST",
  ...
  if (expectSmsDebugCode) {
    ...
    const smsVerifyBeforeBinding = await request("/api/v1/auth/sms/verify", {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  const addPhoneBinding = await request("/api/v1/governance/platform-role-bindings", {
    method: "POST",
  ...
  const smsRequestAfterBinding = await request("/api/v1/auth/sms/request", {
    method: "POST",
    headers: { "content-type": "application/json" },
  ...
  if (expectSmsDebugCode) {
    ...
    const smsVerifyAfterBinding = await request("/api/v1/auth/sms/verify", {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  const invalidCustomRole = await request("/api/v1/governance/custom-roles", {
    method: "POST",
    headers: { "content-type": "application/json", "x-role": "owner" },
    body: JSON.stringify({
  ...
  const globalPolicyDraft = await request("/api/v1/governance/orchestration/policies", {
    method: "POST",
  ...
  const activateGlobalPolicy = await request(`/api/v1/governance/orchestration/policies/${globalPolicyDraft.payload.version}/activate`, {
    method: "POST",
  ...
  const globalPolicies = await getJson("/api/v1/governance/orchestration/policies");
  ...
  const restoreGlobalPolicy = await request("/api/v1/governance/orchestration/policies/restore-initial", {
    method: "POST",
    headers: { "x-role": "owner", "x-user-id": "contract-owner" }
  });
  ...
  const policyExecute = await request("/api/v1/iterations/1/policy-execute", {
    method: "POST",
    headers: { "content-type": "application/json", "x-role": "owner" },
  ...
  const auditAfterRelation = await getJson("/api/v1/governance/audit-logs?limit=10");
  ...
  const snapshotCreate = await request("/api/v1/collab/snapshots", {
    method: "POST",
    headers: { "content-type": "application/json" },
  ...
  const snapshotCreateDenied = await request("/api/v1/collab/snapshots", {
    method: "POST",
  ...
  const snapshotList = await getJson("/api/v1/collab/snapshots?projectId=1");
  assert(Array.isArray(snapshotList) && snapshotList.length >= 1, "snapshot list must include created snapshot");

  const snapshotRestore = await request(`/api/v1/collab/snapshots/${snapshotCreate.payload.id}/restore`, { method: "POST" });
  assert(snapshotRestore.res.status === 200, "restore snapshot should return 200");

  const shareCreate = await request("/api/v1/collab/shares", {
    method: "POST",
    headers: { "content-type": "application/json" },
  ...
  const shareList = await getJson("/api/v1/collab/shares?projectId=1");
  assert(Array.isArray(shareList) && shareList.length >= 1, "share list must include created share");

  const shareAccess = await getJson(`/api/v1/collab/share/${shareCreate.payload.token}`);
  assert(shareAccess.project?.id === 1, "share access should include project");

  const shareComment = await request(`/api/v1/collab/share/${shareCreate.payload.token}/comments`, {
    method: "POST",
    headers: { "content-type": "application/json" },
  ...
  const readOnlyShare = await request("/api/v1/collab/shares", {
    method: "POST",
    headers: { "content-type": "application/json" },
  ...
  const readOnlyComment = await request(`/api/v1/collab/share/${readOnlyShare.payload.token}/comments`, {
    method: "POST",
    headers: { "content-type": "application/json" },
  ...
  const runTemplate = await request(`/api/v1/templates/${templates[0].id}/run`, {
    method: "POST",
    headers: { "content-type": "application/json" },
  ...
  const runTemplateDenied = await request(`/api/v1/templates/${templates[0].id}/run`, {
    method: "POST",
    headers: { "content-type": "application/json", "x-role": "viewer" },
  ...
  const templateRuns = await getJson("/api/v1/templates/runs?projectId=1");
  ...
  const createDeploy = await request("/api/v1/ops/deployments", {
    method: "POST",
    headers: { "content-type": "application/json" },
  ...
  const createDeployDenied = await request("/api/v1/ops/deployments", {
    method: "POST",
    headers: { "content-type": "application/json", "x-role": "viewer" },
  ...
  if (createDeploy.res.status === 200) {
    const deployToRunning = await request(`/api/v1/ops/deployments/${createDeploy.payload.id}/transition`, {
      method: "POST",
    ...
    const deployToSuccess = await request(`/api/v1/ops/deployments/${createDeploy.payload.id}/transition`, {
      method: "POST",
      headers: { "content-type": "application/json", "x-role": "qa" },
  ...
  const deployList = await getJson("/api/v1/ops/deployments?projectId=1");
  ...
  const opsMetrics = await getJson("/api/v1/ops/metrics");
  ...
  assert(invalidCreate.res.status === 400, "POST /api/v1/projects without name should return 400");

  const invalidProjectId = await request("/api/v1/projects/abc/iterations");
  assert(invalidProjectId.res.status === 400, "Invalid project id should return 400");

  const missingProject = await request("/api/v1/projects/999999/iterations");
  assert(missingProject.res.status === 404, "Unknown project id should return 404");

  const projectRepo = await getJson("/api/v1/projects/1/repository");
  ...
  const bootstrapRepo = await request("/api/v1/projects/1/repository/bootstrap", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
      organization: "acme",
  ...
  const repoStatus = await request("/api/v1/projects/1/repository/status");
  ...
  const repoMigrationPlan = await request("/api/v1/projects/1/repository/migration-plan");
  ...
  const repoModeUpdated = await request("/api/v1/projects/1/repository/mode", {
    method: "POST",
    headers: { "content-type": "application/json" },
  ...
  const provisionRepoDryRun = await request("/api/v1/projects/1/repository/provision", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
      ownerType: "org",
  ...
  const scaffoldRepoDryRun = await request("/api/v1/projects/1/repository/scaffold", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
  ...
  const scaffoldRepoReal = await request("/api/v1/projects/1/repository/scaffold", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
  ...
  const publishIterationDryRun = await request("/api/v1/iterations/1/publish", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
  ...
  const bindCodeLink = await request("/api/v1/iterations/1/code-link", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
      branch: "iteration/1-contract",
      commit: "abc123def",
      paths: ["apps/api/v1/src", "apps/web/src"],
  ...
  const getCodeLink = await getJson("/api/v1/iterations/1/code-link");
  assert(getCodeLink.branch === "iteration/1-contract", "iteration code branch should match");

  const traceByRef = await getJson("/api/v1/projects/1/code-trace?ref=abc123def");
  ...
  const createdIteration = await request("/api/v1/projects/1/iterations", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
  ...
  const autoCodeLink = await getJson(`/api/v1/iterations/${createdIterationId}/code-link`);
  assert(typeof autoCodeLink.branch === "string" && autoCodeLink.branch.length > 0, "new iteration should auto link code branch");
  const createdIterationContext = await getJson(`/api/v1/iterations/${createdIterationId}/context`);
  assert(
