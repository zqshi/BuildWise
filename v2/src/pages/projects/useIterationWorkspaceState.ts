...
function deriveChatProcessingState(props: IterationWorkspacePanelProps) {
...
// ── Confirm analysis handler builder ──

function buildConfirmAnalysisHandler(
...
function computeLlmBase(status: string, declared: string[], completed: string[]): number {
...
function mapPrototypeItems(items: string[]): PrototypeElement[] {
...
function buildImageSelectionSummary(
...
function buildPrototypeTree(elements: PrototypeElement[]): Record<string, Record<string, PrototypeElement[]>> {
