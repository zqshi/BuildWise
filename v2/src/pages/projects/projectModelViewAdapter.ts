...
export function toModelRelationsFromView(modelView: ProjectModelViewPayload | null) {
  ...
  return modelView.relations.map((relation) => ({
    id: relation.id,
    ...
    ontologyBasis: modelView.latestSnapshotId || undefined,
    dataBasis: modelView.evidence
  }));
}

export function toModelRuleDescriptionsFromView(modelView: ProjectModelViewPayload | null) {
  if (!modelView || modelView.rules.length === 0) {
    return [];
  }
  return modelView.rules.slice(0, 8).map((rule) => rule.statement || rule.name);
}
...
export function buildModelSummaryHeadline(modelView: ProjectModelViewPayload | null) {
  ...
  const snapshotLabel =
    modelView.latestSnapshotStatus === "published"
      ? "正式快照"
      : modelView.latestSnapshotStatus === "candidate"
        ? "候选快照"
        : modelView.latestSnapshotStatus === "superseded"
          ? "已废弃快照"
          : "尚无快照";
  return `当前建模以${snapshotLabel}为基线，已沉淀领域规则 ${modelView.rules.length} 条、数据实体 ${modelView.entities.length} 个、实体关系 ${modelView.relations.length} 条。`;
}
...
export function buildModelSummaryHighlights(modelView: ProjectModelViewPayload | null, fallbackTrend: string) {
  ...
  const items: string[] = [];
  if (modelView.latestSnapshotId) {
    items.push(`当前快照：${modelView.latestSnapshotId} · ${modelView.latestSnapshotStatus}`);
  } else {
    items.push("当前项目尚未发布正式模型快照");
  }
  if (modelView.reviewTasks.length > 0) {
    items.push(`待确认：${modelView.reviewTasks.slice(0, 2).map((item) => item.title).join("、")}`);
  }
  if (modelView.ontologyTerms.length > 0) {
    items.push(`关键术语：${modelView.ontologyTerms.slice(0, 3).map((item) => item.businessTerm).join("、")}`);
  }
  if (modelView.evidence.length > 0) {
    items.push(`证据：${modelView.evidence.slice(0, 2).join("、")}`);
  }
  if (fallbackTrend) {
...
export function buildModelOperationalSignals(modelView: ProjectModelViewPayload | null): ModelViewOperationalSignals {
  ...
  const reviewTaskCount = modelView.reviewTasks.length;
  ...
  if (modelView.latestSnapshotStatus === "none") {
