...
### P0 — v0.27.0→v0.32.0 已归档；v0.33.0 占位待规划
...
当前活跃 current：v0.33.0（占位待规划，候选：前端 181 lint warnings 清理 / synthesizeTestMatrixOp repair 循环 / 异步 full-cycle job 改造）。v0.32.0 已归档（2026-06-30，投产前卫生收口：T1 后端 12 处 lint warnings 清理[类型守卫替代 !，12→0] + T2 DEPLOY.md 部署 runbook，详见 [v0.32.0-snapshot.md](v0.32.0-snapshot.md)）。v0.31.0 已归档（2026-06-30，按端展示端到端实跑验证：T2 重跑确认 perPlatform 真实产出 + T3 前端按端渲染 DOM 验证[@testing-library/react 真实数据]，详见 [v0.31.0-snapshot.md](v0.31.0-snapshot.md)）
