...
- **KnowledgeSyncService**：KB 变更实时序列化并注入 Agent 上下文
...
### 变更
- Skill 选择逻辑完全重写为基于 SkillRegistry 的选择引擎（stageSkillMap 优先 → 关键词匹配 → 全量兜底）
- Skill 选择上下文构建输出同时包含选择元数据和 SOP 注入内容
- Coach Skill 链路执行从选中 Skill 的 SOP 元数据派生行动建议和检查清单
...
- 移除 workspace SkillsBridge 中旧的硬编码 Skill 选择逻辑
...
### 新增
- 15 个治理技能完整实装（00-orchestrator-sop → 11-product-rd-quality-contract）
- Creative Generator 端到端演示流程
- 前端 UI 风格升级（渐变、动效、响应式）
- 视觉 E2E 对齐校验（Browser Use）

### 变更
- Agent Gateway 统一接入，替代分散 LLM 调用
- 服务层重构：拆分 Coach / Policy / ChangeControl / Agent 四组 Ops
